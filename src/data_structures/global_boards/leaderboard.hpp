...
public:
  ...
  ) : ranking(leaderboard_entry_ptr_comparator<T>{highest_first}),
      file_path(file_path),
      max_size(max_size) 
  {
    load();
   ...
   *
   * @return Tuple of (Player ranking position, or std::nullopt if rejected) and
   * (Removed past index, or std::nullopt if none) The returned past index is
   ...
   */
  std::vector<entry_type> get_range_from_top(std::size_t start,
  ...
  bool load() {
    try {
      ...
      for (const auto& item : j) {
        ...
        entry->timestamp =
            item.at(LEADERBOARD_TIMESTAMP_KEY).get<std::int64_t>();
