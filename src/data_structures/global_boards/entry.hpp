...
struct leaderboard_entry_comparator : entry_comparator {
  ...
  explicit leaderboard_entry_comparator(bool highest_first = true)
...
template <typename T>
inline bool leaderboard_entry_ptr_comparator<T>::operator()(
  const leaderboard_entry<T>* a, 
  const leaderboard_entry<T>* b
) const {
...
inline void to_json(nlohmann::json& j, const entry& e) {
  j = {{"name", e.name}, {"timestamp", e.timestamp}, {"id", e.id}};
}
...
inline void to_json(nlohmann::json& j, const leaderboard_entry<T>& e) {
  j = {{"name", e.name}, {"timestamp", e.timestamp}, {"score", e.score}};
}
