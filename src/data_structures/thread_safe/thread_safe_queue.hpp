...
/**
 * @brief template struct using constexpr sizeof overridable with templates for
 * custom size behaviour Defaults to compile time constexpr sizeof(T)
 ...
 */
template <typename T, bool EnableMaxSize = true, bool EnableMaxBytes = true>
...
public:
   ...
   * @param queue_size_traits<T>::get function used to determine the size of
   * each item
   */
   ...
   */
  virtual ~thread_safe_queue() { stop(); }
  ...
  /**
   * @return the max amount of bytes if max bytes is enabled, 0 if there is no
   * max amount of bytes
   */
  size_t max_bytes() {
  ...
  std::optional<T> wait_and_pop() {
    ...
    waiting.wait(lock, [this] { return stopped || !dequeue.empty(); });
  ...
  std::vector<T> to_vector() const {
    ...
    return {dequeue.begin(), dequeue.end()};
  }

  /**
   * @brief Replaces the queue contents.
   * If the contents are longer than the current size, stops after the current
   * size has been reached (from the start)
 ...
 protected:
  /**
   * @brief returns false if the queue is stopped or if the addition will cause
   * a size overflow. If not, adds the item size to the counter (if counting)
   * and returns true. Call before adding items to the queue to check if can be
   * added
    ...
    if constexpr (EnableMaxBytes) {
      if (maximum_bytes != 0) {
        ...
        if ((current_bytes > maximum_bytes) ||
            (bytes > maximum_bytes - current_bytes)) {
          return false;
        }
