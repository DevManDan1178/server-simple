...
public:
  rate_limiter(
    ...
    cleanup_after_idle_duration(cleanup_after_idle_duration) 
  {
    if (max_tokens <= 0) {
  ...
  bool consume(const std::string& identifier, double cost = 1.0) {
    ...
    if (it == requests.end()) {
      requests[identifier] = {max_tokens - cost, now};
...
private:
  void cleanup_loop() {
    ...
    while (!stop.load()) {
      if (cleanup_cv.wait_for(lock, std::chrono::minutes(1), [this] { return stop.load(); })
      ) {
          break;
      }
