...
public:
  explicit websocket_session(
    ...
    thread_safe_queue<incoming_packet>& incoming_queue
  ...
  void send(std::string message) {
    auto self = shared_from_this();
    boost::asio::post(
      web_socket.get_executor(),
      [this, self, msg = std::move(message)]() mutable {
...
private:
  ...
  void push_incoming_packet(std::string payload) {
    incoming_packet packet{shared_from_this(), std::move(payload)};
    if (!packets_queue_in.try_push_back(std::move(packet))) {
      std::cerr
          << "Closing session: flooding threshold reached (Backpressure).\n";
      boost::beast::error_code ec;
