...
namespace http_parser {
  ...
  using boost_http_response = boost::beast::http::response<boost::beast::http::string_body>;
  using boost_http_status = boost::beast::http::status;

  /**
   * @brief sets response result to bad request and the response text as text
   * plain
   */
  void set_response_bad_request(
   ...
   */
  void set_response_not_found(
    boost_http_response& response, const 
    std::string& response_text = "404 Not Found"
  ) {
    response.set(boost::beast::http::field::content_type, "text/plain");
  ...
  /**
   * @brief sets the response to status unauthorized with response body in text
   * plain
   */
  ...
  /**
   * @brief sets the response to status too many requests with response body in
   * text plain
   */
  void set_response_rate_limited(
    boost_http_response& response,
    const std::string& response_text = "Rate limit exceeded"
  ) {
  ...
  /**
   * @brief sets the response to status service unavailable with response body
   * "Server overloaded"
   */
