...
public:
  http_connection(boost::asio::ip::tcp::socket socket_,
  ...
  void send_response(boost_http_response response, sequence_key sequence_id) {
    ...
    boost::asio::post(strand, [this, self, sequence_id,
                               response = std::move(response)]() mutable {
      responses.emplace(sequence_id, std::make_shared<boost_http_response>(
                                         std::move(response)));
...
private:
  void reject_overloaded(sequence_key sequence_id,
                         unsigned int request_version) {
    auto self = shared_from_this();

    boost::asio::post(strand, [this, sequence_id, request_version, self]() {
