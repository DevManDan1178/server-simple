...
int main(int argc, char** argv) {
    ...
    int worker_thread_count = DEFAULT_WORKER_THREAD_COUNT;
    if (argc > 1) {
    ...
    if (argc > 2) {
        if (!parse_int(argv[2], worker_thread_count)) {
            std::cerr << "Invalid worker thread count.\n";
            return 1;
        }
    }
