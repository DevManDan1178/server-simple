...
struct latency_stats {
  ...
  void reserve(std::size_t count) { samples.reserve(count); }

  void add(uint64_t nanoseconds) { samples.push_back(nanoseconds); }
