...
benchmark_result run_benchmark(
  ...
  for (int producer_id = 0; producer_id < producer_count; ++producer_id) {
    const uint64_t operations = operations_per_producer + (
      static_cast<uint64_t>(producer_id) < remainder ? 1 : 0
    );
  ...
  if (final_consumed != total_operations) {
    std::cerr << "ERROR: expected " << total_operations
              << " consumed operations, but got " << final_consumed << '\n';
...
void print_comparison(
  ...
  std::cout << std::left << std::setw(32) << "Implementation" << std::right
            << std::setw(18) << "Operations/sec" << std::setw(14) << "Relative"
            << '\n';
  ...
  auto print_result = [&](const char* name, const benchmark_result& result) {
    ...
    std::cout << std::left << std::setw(32) << name << std::right
              << std::setw(18) << throughput << std::setw(13) << relative
              << "x\n";
  };
...
void print_usage(const char* program) {
  std::cout << "Usage: " << program
            ...
            << "  operations: " << DEFAULT_OPERATIONS_COUNT << "\n\n"
            << "Example:\n"
            << "  " << program << " 4 4 10000000\n";
}
}  // namespace
...
int main(int argc, char* argv[]) {
  ...
  std::cout << "Thread-Safe Queue Benchmark\n"
            << "===========================\n"
  ...
  thread_safe_queue<value_type, false, false> no_limits_dq(max_items,
                                                           max_bytes);
  thread_safe_queue<value_type, false, true> max_bytes_only_dq(max_items,
                                                               max_bytes);
  thread_safe_queue<value_type, true, false> max_size_only_dq(max_items,
                                                              max_bytes);
  thread_safe_queue<value_type, true, true> max_size_and_bytes_dq(max_items,
                                                                  max_bytes);
  const benchmark_result baseline =
      run_benchmark(mutex_dq, producer_count, consumer_count, total_operations);

  std::cout << "Running thread_safe_queue<false, false>...\n";

  const benchmark_result no_limits = run_benchmark(
      no_limits_dq, producer_count, consumer_count, total_operations);

  std::cout << "Running thread_safe_queue<false, true>...\n";

  const benchmark_result max_bytes_only = run_benchmark(
      max_bytes_only_dq, producer_count, consumer_count, total_operations);

  std::cout << "Running thread_safe_queue<true, false>...\n";

  const benchmark_result max_size_only = run_benchmark(
      max_size_only_dq, producer_count, consumer_count, total_operations);

  std::cout << "Running thread_safe_queue<true, true>...\n";

  const benchmark_result max_size_and_bytes = run_benchmark(
      max_size_and_bytes_dq, producer_count, consumer_count, total_operations);

  print_comparison(baseline, no_limits, max_bytes_only, max_size_only,
                   max_size_and_bytes);
